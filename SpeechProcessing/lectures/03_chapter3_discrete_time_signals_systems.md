...
- Two fundamental building-block sequences underpin everything: the **unit impulse** delta[n] (= 1 at n = 0, else 0) and the **unit step** u[n] (= 1 for n >= 0, else 0). Every signal can be written as a weighted sum of shifted impulses (sifting property).
...
- The input-output relationship of any LTI system is the **convolution sum**: y[n] = x[n] ⊗ h[n] = sum_{m=-inf}^{inf} x[m] h[n-m]. Convolution is commutative, associative, and linear (distributive over addition).
...
```
x[n] = x(t)|_{t=nT} = x(nT),    n = ..., -1, 0, 1, 2, ...       (3.1)
```

x[n] is a **sequence of numbers** ..., x[-1], x[0], x[1], x[2], ... where n is the (dimensionless) time index. The continuous variable t is replaced by the integer n; information between samples is discarded.

Fig. 3.1 (conceptual): the smooth analog curve x(t) is represented by vertical stems at t = ..., -T, 0, T, 2T, ..., each of height x(nT). The stems are the discrete-time signal x[n].
...
```
x[n] = sum_{k=-inf}^{inf}  x[k] * delta[n - k]                   (3.4)
```

This identity is central: it says x[n] = ... + x[-1]*delta[n+1] + x[0]*delta[n] + x[1]*delta[n-1] + ...
...
- Graphically: stems of height 1 at n = 0, 1, 2, ... and zero for n < 0.

**Relationships between delta[n] and u[n]**

```
u[n]     = sum_{k=0}^{inf}  delta[n - k]                          (3.5)
delta[n] = u[n] - u[n - 1]                                        (3.6)
```
...
| Sinusoid | A cos(omega*n + theta) | [A, omega, theta] |

**Example 3.1** — Generate x[n] = A cos(omega*n + theta) with A = 1, omega = 0.3, theta = 1, N = 21 samples (n = 0, ..., 20):
...
```
T{a*x_1[n] + b*x_2[n]} = a*T{x_1[n]} + b*T{x_2[n]} = a*y_1[n] + b*y_2[n]    (3.9)
```
...
**Example 3.2** — y[n] = sum_{k=-inf}^{n} x[k] (running accumulator):

- y_3[n] = sum_{k=-inf}^{n} x_3[k] = sum_{k=-inf}^{n} (a*x_1[k] + b*x_2[k])
  = a * sum_{k=-inf}^{n} x_1[k] + b * sum_{k=-inf}^{n} x_2[k]
  = a*y_1[n] + b*y_2[n]  => **linear**

**Example 3.3** — y[n] = 3*x^2[n] + 2*x[n-3] (contains x^2 term):

- y_3[n] = 3*(a*x_1[n] + b*x_2[n])^2 + 2*(a*x_1[n-3] + b*x_2[n-3])
         = 3*(a^2*x_1^2 + b^2*x_2^2 + 2ab*x_1*x_2) + ...
  ≠ a*(3*x_1^2 + 2*x_1[n-3]) + b*(3*x_2^2 + 2*x_2[n-3])  => **nonlinear**
...
**Example 3.4** — y[n] = sum_{k=-inf}^{n} x[k]:

- y[n - n_0] = sum_{k=-inf}^{n-n_0} x[k]
- x_1[n] = x[n - n_0]: y_1[n] = sum_{k=-inf}^{n} x[k - n_0] = sum_{l=-inf}^{n-n_0} x[l]  (substituting l = k - n_0)
  = y[n - n_0]  => **time-invariant**

**Example 3.5** — y[n] = 3*x[3n] (time-scaling = compression):

- y[n - n_0] = 3*x[3(n - n_0)] = 3*x[3n - 3*n_0]
- y_1[n] = 3*x_1[3n] = 3*x[3n - n_0]
  Since 3*x[3n - n_0] ≠ 3*x[3n - 3*n_0] in general  => **time-variant**

Key insight: any system that compresses or expands the time axis (e.g., y[n] = x[c*n] for integer c ≠ 1) is time-variant.

#### 2.5 Causality

y[n] at time n depends on x[k] only for k <= n (current and past inputs, never future inputs).
...
```
y[n] = x[n] ⊗ h[n] = sum_{m=-inf}^{inf} x[m] * h[n - m]         (3.13)
```
...
```
x[n] ⊗ h[n] = h[n] ⊗ x[n]
= sum_{m=-inf}^{inf} x[m]*h[n-m] = sum_{m=-inf}^{inf} h[m]*x[n-m]   (3.14)
```
...
```
y[n] = x[n] ⊗ (h_1[n] + h_2[n]) = x[n] ⊗ h_1[n] + x[n] ⊗ h_2[n]   (3.16)
```
...
If x[n] has length M and h[n] has length N, then:

```
length of y[n] = x[n] ⊗ h[n]  is  M + N - 1
```
...
```
y[n] = sum_{m=-inf}^{inf} u[m] * (delta[n-m] + 0.5*delta[n-1-m])
     = sum_{m=0}^{inf} (delta[n-m] + 0.5*delta[n-1-m])
     = u[n] + 0.5*u[n-1]
```

Method 2 (general formula then substitute x[n] = u[n]):
```
y[n] = x[n] + 0.5*x[n-1]  =>  y[n] = u[n] + 0.5*u[n-1]
```

Stability check: sum_{n=-inf}^{inf} |h[n]| = |h[0]| + |h[1]| = 1 + 0.5 = 1.5 < inf => **stable**
...
```
y_1[n] = sum_{m=0}^{inf} a^m * u[n-m] = (1 - a^{n+1})/(1-a) * u[n]

y_2[n] = sum_{m=0}^{inf} a^m * u[n-10-m] = (1 - a^{n-9})/(1-a) * u[n-10]
```

Combined result:
```
         { 0,                            n < 0
y[n] =   { (1 - a^{n+1})/(1-a),         0 <= n < 10
         { a^{n-9} * (1 - a^10)/(1-a),  n >= 10
```

sum |h[n]| = 10 < inf => **stable**; h[n] = 0 for n < 0 => **causal**

**Example 3.8** — Finite-length convolution, x[n] = {1, 2, 5, 10} for n=0..3, h[n] = {1, 2, 3, 4} for n=0..3:
...
Result:
```
y[n]: n=0 -> 1, n=1 -> 4, n=2 -> 12, n=3 -> 30, n=4 -> 43, n=5 -> 50, n=6 -> 40, else 0
```

Length = 4 + 4 - 1 = 7 samples (n = 0, ..., 6).

MATLAB: `y = conv(x, h)` gives the correct values. To find the correct starting time index, compute y[0] = x[0]*h[0] = 1*1 = 1 and locate this in the output vector.
...
```
sum_{k=0}^{N} a_k * y[n-k] = sum_{k=0}^{M} b_k * x[n-k]         (3.17)
```
...
**Solving for y[n]** (assuming a_0 ≠ 0):
```
y[n] = (1/a_0) * ( -sum_{k=1}^{N} a_k*y[n-k] + sum_{k=0}^{M} b_k*x[n-k] )    (3.18)
```

**Solving for x[n]** (assuming b_0 ≠ 0):
```
x[n] = (1/b_0) * ( sum_{k=0}^{N} a_k*y[n-k] - sum_{k=1}^{M} b_k*x[n-k] )     (3.19)
```
...
Expand the convolution y[n] = sum_m h[m]*x[n-m] = ... + h[-1]*x[n+1] + h[0]*x[n] + h[1]*x[n-1] + ...
...
For n = 0: y[0] = 0.5*0 + 1 + 0 = 1
For n >= 1: x[n] = x[n-1] = 1, so y[n] = 0.5*y[n-1] + 2

The output converges quickly toward 4 (visible in Fig. 3.6: stem plot shows y rising from 1 at n=0 to approximately 4, settling at 4 for n >= ~10).
...
For a continuous-time signal:  E = integral_{-inf}^{+inf} |x(t)|^2 dt

For a discrete-time signal:
```
E[n] = sum_{n=-inf}^{inf} |x[n]|^2
```
...
For a continuous-time signal:  P = lim_{T->inf} (1/T) * integral_{-T/2}^{T/2} |x(t)|^2 dt

For a discrete-time signal:
```
P[n] = lim_{N->inf} (1/(2N+1)) * sum_{n=-N}^{N} |x[n]|^2
```
...
|---|---|---|
| Energy E | 0 < E < inf | E = inf |
| Average power P | P = 0 | 0 < P < inf |
...
**Signal (a):** x(t) = 5 for -2 <= t <= 2, else 0 (single rectangular pulse):
```
Instantaneous power: P_x(t) = 25  for -2 <= t <= 2, else 0
Energy: E_x = integral_{-2}^{2} 25 dt = 100    (finite)
Average power: P_x = lim_{T->inf} (1/T) * 100 = 0
```
=> **Energy signal**

**Signal (b):** z(t) = periodic repetition of the above pulse with fundamental period 8:
```
Average power: P_z = (1/8) * integral_{-2}^{2} 25 dt = 100/8 = 12.5   (finite)
Energy: E_z = integral_{-inf}^{+inf} |z(t)|^2 dt = inf
```
=> **Power signal**
...
- **u[n] (unit step)** — equals 1 for n >= 0, zero for n < 0.
...
- **Causal system** — y[n] depends only on x[k] for k <= n; for LTI: h[n] = 0 for n < 0.
- **BIBO stable system** — bounded input => bounded output; for LTI: sum|h[n]| < inf.
- **Impulse response h[n]** — output of an LTI system when input = delta[n].
- **Convolution sum** — y[n] = x[n] ⊗ h[n] = sum_m x[m]*h[n-m]; the universal LTI input-output formula.
...
- **Energy signal** — 0 < E = sum|x[n]|^2 < inf; average power = 0.
- **Power signal** — 0 < P = lim (1/(2N+1)) sum|x[n]|^2 < inf; energy = inf.
...
2. **Test linearity** of a given system using the standard three-input approach (x_3 = a*x_1 + b*x_2, check if T{x_3} = a*T{x_1} + b*T{x_2}). Know that any system with x^2, |x|, or x*y-type terms is nonlinear.

3. **Test time-invariance**: substitute x_1[n] = x[n - n_0], compute y_1[n] = T{x_1[n]}, compare with y[n - n_0]. Systems that compress/expand the time axis (y[n] = x[c*n], c ≠ 1) are always time-variant.

4. **State the causality and stability conditions for LTI systems**: h[n] = 0 for n < 0 (causality); sum_{n=-inf}^{inf} |h[n]| < inf (stability/BIBO).
...
9. **Classify a signal as energy or power**: compute E = sum|x[n]|^2 and P = lim (1/(2N+1)) sum|x[n]|^2; apply the mutual exclusion rules.

---

## Pitfalls

- **delta[n] vs delta(t)**: delta[n] equals exactly 1 at n = 0. The continuous Dirac delta is undefined at 0. Do not confuse them — and do not write delta[n] = infinity at n = 0.
...
- **BIBO stability for LTI is about |h[n]|, not |h[n]|^2**: the condition is absolute summability (sum |h[n]| < inf), not square-summability (which is the energy condition).
