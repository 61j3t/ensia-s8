...
- **Fundamental trade-off:** Long window → better frequency resolution, poor time resolution (narrowband spectrogram, horizontal striations at harmonics). Short window → better time resolution, poor frequency resolution (wideband spectrogram, vertical striations at pitch periods). Governed by the **uncertainty principle**: $\Delta t \cdot \Delta f \geq \frac{1}{4\pi}$.
...
- **Practical speech parameters:** Frame size 20–40 ms, overlap 5–10 ms, Hamming window standard; wideband uses $L \approx 6$ ms (96 samples at 16 kHz), narrowband uses $L \approx 60$ ms (960 samples at 16 kHz).
...
For a rectangular window: $w[n] = 1$ for $0 \leq n \leq M$, and $w[n] = 0$ elsewhere.

The windowed signal is: $x_w[n] = w[n] \cdot x[n]$
...
- $e^{-j(2\pi k n/N)}$ = the DFT kernel (green)
...
For **speech**: frame changes occur at 20–40 ms timescales, so target T = 20–40 ms with overlap 5–10 ms. Window size in ms: $T = M \cdot T_s$, where $T_s = 1/f_s$ is the sampling period.
...
**Example — sea lions:** The time waveform shows complex bursting calls over 40 seconds. The global spectrum (DFT) shows energy concentrated below ~200 Hz. The magnitude spectrogram (linear) is nearly all dark because most energy is at low frequencies and the dynamic range overwhelms detail. The **dB spectrogram** reveals rich harmonic structure: horizontal bands of energy corresponding to harmonics of $f_0$, with energy bursting and decaying in a periodic rhythmic pattern across the full time axis.
...
- **Narrowband spectrogram** (L = 60 ms, 960 samples, Hamming, N = 1024, R = 96 samples, 235 frames): horizontal striations at each harmonic multiple of $f_0$; fundamental frequency and all harmonics clearly resolved; time localisation poor (formant transitions are blurred); voiced vs. unvoiced regions still visible.
...
- The DFT has high frequency resolution: individual harmonics at $f_0$, $2f_0$, $3f_0$ are resolved as separate spectral lines.
...
**Practical guideline for speech:** speech spectral characteristics change on a 20–40 ms timescale, so the analysis window T should target 20–40 ms. Overlap should be 5–10 ms. Window size in samples: $M = T \times f_s$.
...
| Fundamental $f_0$ | Not directly readable | Directly readable |
...
**Narrowband spectrogram:** individual harmonics resolved in voiced regions; formant frequencies still evident as regions of high harmonic energy; usually can see $f_0$ directly; unvoiced regions show no strong structure.
...
**Consequences:**
- Increasing frame size → better $\Delta f$ → worse $\Delta t$ (the "blurring disk" in time-frequency space becomes wider in time and narrower in frequency).
- Decreasing frame size → better $\Delta t$ → worse $\Delta f$.
...
- **Spectrogram:** The squared magnitude of the STFT, $Y[m, k] = |S[m, k]|^2$, displayed as a 2-D energy map.
...
- **Fundamental frequency $f_0$:** The lowest harmonic of a periodic voiced sound; harmonics are at $f_n = n \cdot f_0$.
- **Formants:** Resonance peaks of the vocal tract filter, visible as broad horizontal bands in the wideband spectrogram.
- **Frequency bin:** One output sample of the DFT corresponding to frequency $k \cdot F_s / N$.
- **Uncertainty principle:** $\Delta t \cdot \Delta f \geq \frac{1}{4\pi}$ — the inescapable lower bound on the product of time and frequency localisation.
...
- **Hamming window:** $w[n] = \left(0.54 - 0.46\cos\left(\frac{2\pi n}{M}\right)\right) \cdot w_R[n]$; the standard default for speech analysis.
- **Hann window:** $w[n] = \frac{1}{2}\left(1 - \cos\left(\frac{2\pi n}{M}\right)\right) \cdot w_R[n]$; good leakage suppression.
- **Blackman window:** $w[n] = \left(0.42 - 0.5\cos\left(\frac{2\pi n}{M}\right) + 0.08\cos\left(\frac{4\pi n}{M}\right)\right) \cdot w_R[n]$; best leakage suppression, widest main lobe.
...
2. **Write and annotate the STFT formula** $S[m, k] = \sum_{n} x[n] \cdot w[n-mH] \cdot e^{-j2\pi kn/N}$. Identify each term (time index m, frequency index k, hop size H, window w, DFT kernel) and explain what the summation is computing.
...
4. **State and apply the uncertainty principle** $\Delta t \cdot \Delta f \geq \frac{1}{4\pi}$. Explain what it means for the STFT and why narrowband and wideband spectrograms cannot simultaneously achieve both resolutions.
...
6. **Define the spectrogram** $Y[m,k] = |S[m,k]|^2$. Explain the dB conversion ($10\log_{10} Y$ or $20\log_{10}|S|$) and why dB display is necessary in practice.
...
- **Spectrogram ≠ STFT.** The STFT S[m,k] is complex. The spectrogram $Y[m,k] = |S[m,k]|^2$ is real and discards phase. You cannot recover the signal from the spectrogram alone (phase information is lost).
...
- **The uncertainty principle applies to the STFT, not just quantum mechanics.** $\Delta t \cdot \Delta f \geq \frac{1}{4\pi}$ is a purely mathematical consequence of Fourier analysis; it is not a technological limitation — it cannot be circumvented by better hardware or algorithms.
...
- **The good window length for speech depends on $f_s$ (Nyquist).** The rule $T = M \times T_s = M/f_s$ ties the window duration in milliseconds to the sampling rate. At $f_s = 16$ kHz, 6 ms corresponds to 96 samples; at $f_s = 8$ kHz, 6 ms is only 48 samples.
