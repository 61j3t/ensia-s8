...
- A cepstrum plot of a voiced vowel shows a smooth low-quefrency bulge (vocal tract) plus a sharp peak at q0 (pitch period); pitch frequency = Fs / q0.
...
In words:
1. Take the DFT of the time-domain signal x[n] to get the spectrum F{x[n]}.
2. Take the magnitude |F{x[n]}| (log spectrum when logs are applied).
...
where:
- e[n] = excitation signal (voiced speech: periodic impulse train at pitch period T0; unvoiced: white noise)
...
- The **excitation** e[n] has rapid harmonic structure (pitch and its overtones) → maps to **high quefrency** components c_e[n], with a peak at the pitch period q0.
...
Step by step:
1. **Linear transform (DFT)**: convolution s[n] = e[n]*h[n] becomes multiplication S(k) = E(k)·H(k).
...
- **Low-pass lifter** (keep n <= Nc): isolates vocal tract component c_h[n].
- **High-pass lifter** (keep n > Nc): isolates excitation component c_e[n].
- Nc is the **cutoff quefrency** that separates the two regions.
...
- A **sharp, prominent peak at q0** (e.g., q0 = 8 ms → F0 = 1/0.008 = 125 Hz): this is the pitch period. For a female voice, q0 ~ 4 ms → F0 ~ 250 Hz.
- **No peak** in the high quefrency region for unvoiced speech (the source is noise, not periodic).

The cutoff quefrency Nc is placed between 2 ms and q0 so that:
- Everything below Nc (low-pass) = vocal tract.
- The peak at q0 and above (high-pass) = excitation (pitch).
...
4. The quefrency of the peak = pitch period q0.
5. Convert to fundamental frequency: F0 = Fs / q0.

Example result: q0 = 4.18 ms → F0 = 239.29 Hz.
...
**Problem**: speech spectra exhibit a natural **spectral tilt** — high frequencies contain less energy than low frequencies, caused by the glottal pulse shape (roughly -6 dB/octave roll-off above F0).
...
Transfer function: H(z) = 1 - alpha · z^{-1} (a simple differentiator).
...
The windowed frame x_w[n] = x[n] · w[n] tapers to zero at both ends, preventing discontinuities from producing high-frequency artifacts in the FFT.
...
```
X_t[k] = DFT{ x_t[n] }    (complex-valued, k = 0, 1, ..., N-1)
```
...
Formal definition of filter m (m = 1, 2, ..., M):
...
```
f[m] = (N/Fs) · B^{-1}( B(f_l) + m · (B(f_h) - B(f_l)) / (M+1) )
```

where B is the Mel-scale function (Mel(f) above), B^{-1} is its inverse, f_l and f_h are the lowest and highest frequencies of the filterbank, Fs is the sampling frequency, and N is the FFT size.
...
```
MF_t[m] = log10( sum_{k=0}^{N-1} H_{t,m}[k] · |X[k]|^2 )    for 1 <= m <= M
```
...
Key points:
- Typically N_MFCC = 12 and M = 24 (fewer MFCCs than Mel filters).
...
- Coefficients at d = 1, 2, ..., 12 capture progressively finer spectral envelope shape.
- The **energy feature** (MFCC[0]) is often replaced by the log frame energy:

```
logEng = log( sum_t |x[k]|^2 )
```
...
```
delta_t[d] = (sum_{tau=1}^{R} tau · (c_{t+tau}[d] - c_{t-tau}[d])) / (2 · sum_{tau=1}^{R} tau^2)
```
...
1. **Pre-emphasis**: x[n] = x'[n] - 0.97·x'[n-1] (flatten spectrum).
2. **Window**: multiply 400-sample frame by Hamming window.
3. **FFT**: compute X[k] for k = 0..399.
4. **Power spectrum**: P[k] = |X[k]|^2.
5. **Mel filterbank**: compute f[0]..f[7] (center frequencies for M=6 filters need M+2=8 values) using the Mel formula with f_l=0, f_h=8000 Hz.
6. **Filter energies**: MF[m] = log10(sum_k H_m[k]·P[k]) for m=1..6.
7. **DCT**: MFCC[d] = sum_{m=1}^{6} MF[m] · cos(pi·d·(m-0.5)/6) for d=1..12 (but only d <= M here, so d=1..6 meaningful).
...
- **Cutoff quefrency (Nc)** — the lifter threshold separating low-quefrency (vocal tract) from high-quefrency (excitation) cepstral regions.
- **Spectral tilt** — the natural -6 dB/octave fall-off of speech energy at high frequencies, caused by the glottal pulse.
- **Pre-emphasis** — first-order high-pass filter applied to flatten spectral tilt; x[n] = x'[n] - alpha·x'[n-1], alpha ~ 0.97.
- **Mel scale** — perceptual frequency scale: Mel(f) = 2595·log10(1 + f/700); linear below 1 kHz, logarithmic above.
...
- **DCT (Discrete Cosine Transform)** — decorrelates filterbank energies; reduces M filter outputs to N_MFCC coefficients.
...
- **Pitch period (q0)** — the quefrency of the main cepstral peak; F0 = Fs / q0.

---

## Exam targets

1. **Define the cepstrum** mathematically and explain the meaning of "quefrency." Write: c[n] = F^{-1}{log|F{x[n]}|}.
...
4. **Describe homomorphic deconvolution** with the three-step diagram: s[n] → F → log|·| → F^{-1} → c[n]. State what liftering is, write the ideal low-pass lifter formula, and explain how to recover the spectral envelope H(k).

5. **Interpret a cepstrum plot** of a voiced vowel: identify the vocal tract region (low quefrency, smooth decay), the pitch peak (sharp spike at q0), and explain how to estimate F0 from q0.

6. **Write the Mel scale formula** Mel(f) = 2595·log10(1+f/700) and sketch the curve. Explain why it is linear below 1 kHz and logarithmic above.
...
8. **Write the triangular filterbank formula** H_{t,m}[k] for a bank of M filters and the center-frequency formula f[m] in FFT bins. Explain how f_l, f_h, N, Fs, M relate.

9. **Write the DCT formula** for MFCCs: MFCC_t[d] = sum_{m=1}^M MF_t[m]·cos(pi·d·(m-1/2)/M), and explain why the DCT is applied instead of a full inverse DFT.
...
- **Real vs. complex cepstrum formulas**: real cepstrum uses log|X(k)| (magnitude only); complex cepstrum uses log X(k) = log|X(k)| + j·arg X(k). Do not confuse the two.
...
- **Pre-emphasis is applied before FFT**, not after. Its transfer function is H(z) = 1 - alpha·z^{-1} (a highpass differentiator). The typical value alpha = 0.97 is standard; some variants use 0.95.
...
- **F0 from cepstrum**: F0 = Fs / q0 (NOT 1/q0 alone — you must divide by sampling frequency). q0 is in samples; Fs converts to Hz.
...
- **Liftering cutoff Nc must be between the vocal tract region and q0** — if Nc > q0, you will include the pitch peak in the "vocal tract" estimate, ruining the separation.
