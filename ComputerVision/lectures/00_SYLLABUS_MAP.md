...
**Recurring threads**: the gradient ∇I (edges P7/P8, corners P10, SIFT P11); scale & invariance (features P7, SIFT P11/P12); the spatial ↔ frequency duality (P5 convolution, P6 Fourier); voting/robust fitting (Hough + RANSAC P9).
...
- **Digital sensing pipeline**: photon → photoelectric charge `Q ∝ photons × exposure` → voltage → ADC → digital number; CCD vs CMOS.
...
- **Basic transforms**: negative `s=L−1−r`; gain/bias `s=αr+β`; log `s=c·log(1+r)`; **power-law/gamma** `s=c·r^γ` (γ<1 brightens, γ>1 darkens).
- **Gamma correction**: pre-distort by `1/γ_display` (≈2.2) for perceptual linearity.
- **Contrast stretching** (piecewise-linear) & thresholding.
- **Histogram processing**: histogram `h(r_k)=n_k`; **histogram equalization** `s_k=(L−1)·CDF(r_k)`; **Otsu** automatic threshold (minimize within-class variance).

## Part 5 — Spatial Filtering (49 pp)
- **Neighborhood operator**: `g(x,y)=Σ w(s,t)·f(x+s,y+t)`; linear + shift-invariant ⇒ convolution.
- **Correlation vs convolution**: convolution flips the kernel 180°.
- **Smoothing**: box average vs **Gaussian** (σ controls blur, fewer artifacts).
- **Sharpening**: unsharp masking; **Laplacian** `∇²f` (isotropic 2nd derivative); `g=f+c·∇²f`.
...
- **Convolution theorem**: `f∗h ↔ F×H` (and the dual). Pipeline: FT → multiply by `H(u,v)` → inverse FT.
...
- **Gradient detection**: `∇I`, magnitude `‖∇I‖`, direction `arctan(I_y/I_x)`; Sobel/Prewitt/Roberts; thresholding (single vs hysteresis).
- **Laplacian detection**: `∇²I`, edges = zero-crossings (location only, no orientation).
- **Noise**: smooth before differentiating; Derivative-of-Gaussian `∇(G_σ∗f)=(∇G_σ)∗f`; **LoG** (Mexican hat).
...
- **LoG / Marr-Hildreth**: `∇²[G_σ∗I]`, zero-crossings.
- **Canny — 3 optimality criteria**: low error, good localization, single response.
- **Canny — 4 stages**: Gaussian smooth → Sobel gradient → **non-maximum suppression** → **double-threshold hysteresis** (`T_low < T_high`).
- **Evaluation**: Precision/Recall/F1, Pratt's FOM; BSDS500 dataset.
- **Line/curve fitting**: least-squares (vertical vs perpendicular/PCA); polynomial via pseudo-inverse `a=(XᵀX)⁻¹Xᵀy`.

## Part 9 — The Hough Transform (& RANSAC) (50 pp)
- **History**: Hough 1962; Duda & Hart 1972.
- **Line detection**: slope-intercept `(m,c)` (m unbounded — problematic) vs **polar** `ρ = x cosθ + y sinθ` (bounded). **Point ↔ curve duality**; an **accumulator** array votes.
- **Circle detection**: `(x−a)²+(y−b)²=r²`; 2-D accumulator `A(a,b)` (r known) or 3-D `A(a,b,r)` (r unknown).
...
- **Structure tensor M** (2×2 weighted sum of gradient outer products); eigenvalues `λ1,λ2` = change strength in principal directions.
- **Harris response**: `R = det(M) − k·trace(M)²` (k≈0.04-0.06); R>0 corner, R<0 edge, |R|≈0 flat (avoids eigendecomposition).
- **Algorithm**: gradients → build M → R → threshold → non-max suppression.
- **Invariance**: translation + rotation (full); scale + illumination (partial). Variants: **Shi-Tomasi** (`min(λ1,λ2)>T`), Förstner.
...
- **Scale space**: `L(x,y,σ)=G(x,y,σ)∗I`; **Difference-of-Gaussians** `DoG ≈ (s−1)σ²∇²G` approximates scale-normalized LoG.
- **Gaussian/DoG pyramid**: within-octave blur (σ) + octaves (halved resolution).
...
- **Invariance**: window rotated to keypoint orientation; bins relative to dominant orientation; window scales with σ.
- **Normalization**: L2-normalize → clip components >0.2 → L2-normalize (illumination invariance).
- **Matching**: Euclidean distance + **Lowe's ratio test** (`d₁/d₂ < 0.8`); NN search (linear or indexed).
...
- **Classical OF**: **Lucas-Kanade** (local least-squares, structure tensor) vs **Horn-Schunck** (global energy + smoothness α); pyramidal for large motion.
...
- **The gradient ∇I** everywhere: edges (P7/P8), corners/structure tensor (P10), SIFT (P11).
