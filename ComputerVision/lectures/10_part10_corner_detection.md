...
- The key insight: classify regions by analysing the **distribution of gradient vectors** ($I_x$, $I_y$) inside a local window — flat regions have a tight isotropic cluster, edges a thin elongated ellipse, corners a wide isotropic ellipse.
- The **structure tensor M** (second-moment matrix) is a 2×2 weighted sum of outer products of gradient vectors over a local window; its **eigenvalues $\lambda_1$, $\lambda_2$** measure the strength of intensity change along the two principal gradient directions.
- The **Harris Corner Response** $R = \det(M) - k \cdot \text{tr}(M)^2$ avoids computing eigenvalues explicitly; $R > \text{threshold}$ signals a corner, $R < 0$ signals an edge, $|R|$ small signals a flat region.
...
- Variants: **Shi-Tomasi** (corner if $\min(\lambda_1, \lambda_2) > \text{threshold}$ — simpler, no heuristic k); **Forstner** (requires both eigenvalues large AND their ratio not too extreme, for precise localisation).
...
**Key observation (slide 5–7):** Looking at the partial derivatives $I_x = \partial I/\partial x$ and $I_y = \partial I/\partial y$ for the three region types:

- **Flat region:** Both $I_x$ and $I_y$ are uniformly small (near-zero noise). In the $(I_x, I_y)$ scatter plot, points cluster tightly near the origin in a small circle.
- **Edge region:** One of $I_x$ or $I_y$ is large while the other is near-zero (depending on edge orientation). The scatter is an elongated ellipse pointing along one axis.
- **Corner region:** Both $I_x$ and $I_y$ take significant non-zero values in different sub-areas of the window. The scatter is a wide, roughly circular (isotropic) spread.
...
The eigenvalues $\lambda_1$ and $\lambda_2$ of M give the **strength of intensity change in the two orthogonal principal directions**.

| $\lambda_1$, $\lambda_2$ values | Region type | Gradient scatter shape |
|---|---|---|
| Both small ($\lambda_1 \approx \lambda_2 \approx 0$) | **Flat** | Small circle near origin |
| One large, one small ($\lambda_1 \gg \lambda_2$ or $\lambda_2 \gg \lambda_1$) | **Edge** | Thin elongated ellipse |
| Both large ($\lambda_1 \approx \lambda_2$, both high) | **Corner** | Large, roughly circular ellipse |

**Decision diagram (the $\lambda_1$–$\lambda_2$ space):**
Imagine a plot with $\lambda_1$ on the horizontal axis and $\lambda_2$ on the vertical axis:
- **Bottom-left region** (both small): flat — no features.
- **Lower-right region** ($\lambda_1 \gg \lambda_2$): edge along the y-direction.
- **Upper-left region** ($\lambda_2 \gg \lambda_1$): edge along the x-direction.
...
1. **Compute image gradients** at every pixel:
   - $I_x(x,y) = \partial I/\partial x$ (e.g. using Sobel or finite differences)
   - $I_y(x,y) = \partial I/\partial y$
...
|---|---|---|---|
| **Harris (1988)** | $R = \lambda_1\lambda_2 - k(\lambda_1+\lambda_2)^2 > T$ | Combines eigenvalues with an empirical heuristic | Sensitive to parameter k |
| **Shi-Tomasi (1994)** | $\min(\lambda_1, \lambda_2) > \text{threshold}$ | Smallest eigenvalue must be large enough | Simpler, no heuristic k; more robust for tracking |
...
**Shi-Tomasi detail:** Instead of the Harris response R, Shi and Tomasi declare a point a corner if $\lambda_{\min} > \text{threshold}$, where $\lambda_{\min} = \min(\lambda_1, \lambda_2)$. This directly asks "is the weakest direction also strong?" — a corner must have strong gradients in all directions. Simpler and avoids the k parameter entirely.

**Forstner detail:** A point is a good feature if (a) both eigenvalues are large (like Harris and Shi-Tomasi) and (b) their ratio $\lambda_1/\lambda_2$ is not too extreme (ensuring isotropy — the feature is well-localised in all directions). Forstner was designed specifically for precise spatial localisation in photogrammetry.
...
- **Flat region** — region with no significant gradient in any direction; both $\lambda_1$, $\lambda_2$ small.
- **Edge region** — region with strong gradient in one direction; one eigenvalue large, one small.
- **Structure tensor / Second-moment matrix M** — 2×2 symmetric matrix summarising the distribution of gradient vectors in a local window; $M = \sum w(x,y) \begin{bmatrix} I_x^2 & I_x I_y \\ I_x I_y & I_y^2 \end{bmatrix}$.
- **Weighting function $w(x,y)$** — typically a Gaussian; gives more importance to pixels near the window centre.
- **Eigenvalues $\lambda_1$, $\lambda_2$ of M** — strengths of intensity change in the two principal (orthogonal) gradient directions.
- **Corner Response Function R** — scalar score $R = \det(M) - k\cdot\text{tr}(M)^2$; large positive = corner, negative = edge, near zero = flat.
- **$\det(M)$** — $\lambda_1 \cdot \lambda_2$; product of eigenvalues.
- **$\text{tr}(M)$** — $\lambda_1 + \lambda_2$; sum of eigenvalues.
...
- **Shi-Tomasi criterion** — a corner if $\min(\lambda_1, \lambda_2) > \text{threshold}$; avoids the k heuristic.
- **Forstner detector** — detects corners requiring both eigenvalues large and their ratio balanced.
- **PCA on gradients** — conceptual interpretation of what M computes; $\lambda_1$/$\lambda_2$ are the principal component variances of the gradient vector cloud.
...
2. **Explain the gradient distribution perspective:** describe what the $(I_x, I_y)$ scatter plot looks like for flat, edge, and corner regions, and what $\lambda_1$/$\lambda_2$ signify geometrically.
...
- **Confusing $\det(M)$ and $\text{tr}(M)$ in the formula.** The formula is $R = \det(M) - k\cdot\text{tr}(M)^2$ — det comes first, trace is squared. Not the other way round.
...
- **The sign of R for edges.** When one eigenvalue dominates (edge), det $\approx$ 0 but trace is large, so $-k\cdot\text{tr}^2$ dominates and R is negative.
...
- **Shi-Tomasi vs Harris:** Shi-Tomasi does NOT use R. It directly thresholds $\min(\lambda_1, \lambda_2)$. This means it does need to compute eigenvalues explicitly (or equivalent), unlike Harris which avoids that via det and trace.
