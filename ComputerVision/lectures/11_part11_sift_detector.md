...
- The **scale space** $L(x,y,\sigma) = G(x,y,\sigma) \ast I(x,y)$ is searched for extrema via the **Difference-of-Gaussians (DoG)**, which efficiently approximates the Normalized Laplacian of Gaussian (NLoG).
...
- Each keypoint is assigned a **characteristic scale** ($\sigma^{\ast}$ at which the NLoG response peaks) and a **principal orientation** from a weighted gradient histogram, giving full invariance to scale and rotation.
...
1. Locate the blob (position $x^{\ast}$, $y^{\ast}$)
2. Determine its size (scale $\sigma^{\ast}$)
...
- Row 4: $(\partial^2 n_{\sigma}/\partial x^2) \ast f(x)$ — convolution result (unnormalized)
- Row 5: $\sigma^2 \cdot (\partial^2 n_{\sigma}/\partial x^2) \ast f(x)$ — $\sigma$-normalized result
...
- $x^{\ast}$ = position of the extremum → **blob position**
- $\sigma^{\ast}$ = $\sigma$ at which the extremum occurs → **characteristic scale** = **blob size**
...
2. Compute $\sigma^2 \cdot (\partial^2 n_{\sigma}/\partial x^2) \ast f(x)$ at $k$ different scales $(\sigma_0, \sigma_1, \ldots, \sigma_k)$
3. Find $(x^{\ast}, \sigma^{\ast}) = \arg\max_{(x,\sigma)} \left|\sigma^2 \cdot \frac{\partial^2 n_{\sigma}}{\partial x^2} \ast f(x)\right|$
   - $x^{\ast}$ = the blob position
   - $\sigma^{\ast}$ = the characteristic scale (blob size)
...
3. Find $(x^{\ast}, y^{\ast}, \sigma^{\ast}) = \arg\max_{(x,y,\sigma)} \left|\sigma^2 \cdot \nabla^2 n_{\sigma} \ast I(x,y)\right|$
   - $(x^{\ast}, y^{\ast})$ = the blob position in the image
   - $\sigma^{\ast}$ = the characteristic scale (blob size)
...
After localizing the keypoint at $(x^{\ast}, y^{\ast}, \sigma^{\ast})$, SIFT assigns a **principal orientation** to make descriptors rotation-invariant.
...
- Position $(x^{\ast}, y^{\ast})$
- Characteristic scale $\sigma^{\ast}$ (from the DoG extremum)
- Principal orientation $\theta^{\ast}$ (from the gradient histogram)
...
- **Scale invariance:** Two views of the same blob at different scales produce keypoints with different $\sigma^{\ast}$ values. The ratio $\sigma_1/\sigma_2$ equals the actual scale ratio between the two images. The descriptor region is normalized by $\sigma^{\ast}$ before computing.

- **Rotation invariance:** Two views of the same blob at different orientations produce keypoints with different $\theta^{\ast}$ values. The descriptor is computed in the rotated frame, so rotation differences are factored out.

**Visualization (slides 47–48):** Two images of the same DVD cover, one upright and one rotated ~45°. The orange circle marks the same large blob in both images. In the upright image the orientation arrow points upward; in the rotated image the arrow has rotated accordingly. After "undoing" the rotation (normalizing by $\theta^{\ast}$), both blobs produce the same canonical patch.
...
| **Scale space** | $S(x,y,\sigma) = n(x,y,\sigma) \ast I(x,y)$; the family of images obtained by convolving $I$ with Gaussians of increasing $\sigma$ |
...
| **Characteristic scale** | $\sigma^{\ast}$ at which the NLoG (or DoG) response is extremized; proportional to blob size |
...
| **Principal orientation $\theta^{\ast}$** | The dominant gradient direction assigned to a keypoint; enables rotation invariance in descriptor computation |
...
4. **Derive the 1D blob detection principle.** Explain the $\sigma$-normalized 2nd derivative, what "characteristic scale" means, and why $\sigma^2$ normalization is necessary. Write the algorithm formally: given $f(x)$, compute at $k$ scales, find $(x^{\ast}, \sigma^{\ast}) = \arg\max \left|\sigma^2 (\partial^2 n_{\sigma}/\partial x^2) \ast f(x)\right|$.

5. **Write the NLoG formula in 2D** and the 2D blob detection algorithm. Write the scale-space equation $S(x,y,\sigma) = n(x,y,\sigma) \ast I(x,y)$.
...
- **Characteristic scale is proportional to blob SIZE, not to position.** $\sigma^{\ast}$ tells you how large the blob is, not where it is. Position comes from $(x^{\ast}, y^{\ast})$.
