...
- The **scale space** L(x,y,σ) = G(x,y,σ) * I(x,y) is searched for extrema via the **Difference-of-Gaussians (DoG)**, which efficiently approximates the Normalized Laplacian of Gaussian (NLoG).
- The Gaussian pyramid is organized into **octaves** (each halving image resolution) with several **blur levels** (controlled by σ) per octave; DoG images are computed between adjacent blur levels.
...
- Each keypoint is assigned a **characteristic scale** (σ* at which the NLoG response peaks) and a **principal orientation** from a weighted gradient histogram, giving full invariance to scale and rotation.
- Knowing scale and orientation allows rotation and scale variations between images to be **undone**, enabling reliable matching; the ratio of blob sizes between two views equals σ₁/σ₂.
...
Define the **normalized (σ-normalized) 2nd derivative** of the Gaussian-smoothed signal:

```
σ² · (∂²n_σ/∂x²) * f(x)
```

where n_σ is the 1D Gaussian with standard deviation σ, and * denotes convolution.

**Why normalize by σ²?** Without normalization, the response magnitude falls off as σ increases (larger blobs produce weaker raw 2nd derivatives). The σ² factor compensates, making the response comparable across scales.

**Key result from the 1D blob analysis (slides 20–27):**

The course shows three 1D blobs (A, B, C) of increasing width. For each blob, at a fixed σ:
- Row 1: f(x) — the original signal
- Row 2: n_σ — Gaussian at that σ
- Row 3: ∂²n_σ/∂x² — 2nd derivative of Gaussian (a "W" shaped kernel)
- Row 4: (∂²n_σ/∂x²) * f(x) — convolution result (unnormalized)
- Row 5: σ² · (∂²n_σ/∂x²) * f(x) — σ-normalized result

As σ increases across the slides, for a given blob:
- When σ is small relative to the blob width: the convolution produces oscillating responses, and the normalized 2nd derivative shows a **clear minimum** (negative peak) right at the blob center.
- When σ matches the blob width: the minimum is strongest.
- When σ is large relative to the blob: the response flattens out (the blob is "subsumed" by the large Gaussian).

**Conclusion:** Each blob is detected as a **minimum or maximum of the σ-normalized 2nd derivative in the (x, σ) joint space.** The blob is localized at:
- x* = position of the extremum → **blob position**
- σ* = σ at which the extremum occurs → **characteristic scale** = **blob size**

**Characteristic scale definition:** The σ at which the σ-normalized 2nd derivative of the signal attains its minimum/maximum.

**Critical property:** The characteristic scale is **proportional to the blob size**:

```
size of blob A   σ*_A       size of blob B   σ*_B
—————————————— = ————    ,  —————————————— = ————
size of blob B   σ*_B       size of blob C   σ*_C
```

**1D blob detection algorithm:**

1. Given a 1D signal f(x)
2. Compute σ² · (∂²n_σ/∂x²) * f(x) at k different scales (σ₀, σ₁, ..., σ_k)
3. Find (x*, σ*) = arg max_{(x,σ)} |σ² · (∂²n_σ/∂x²) * f(x)|
   - x* = the blob position
   - σ* = the characteristic scale (blob size)
...
where n_σ is a 2D isotropic Gaussian. Visually:
- The Gaussian n_σ is a smooth bell-shaped surface (peak in center)
- The LoG ∇²n_σ is a "Mexican hat" shape (negative central region, positive ring around it)
- The NLoG σ²∇²n_σ is the σ-normalized version with preserved amplitude across scales

**2D blob detection algorithm:**

1. Given an image I(x,y)
2. Convolve I with the NLoG at k different scales (σ₀, σ₁, ..., σ_k)
3. Find (x*, y*, σ*) = arg max_{(x,y,σ)} |σ² · ∇²n_σ * I(x,y)|
   - (x*, y*) = the blob position in the image
   - σ* = the characteristic scale (blob size)

**Scale space S(x,y,σ):** A stack of images created by filtering with different values of σ:

```
S(x, y, σ) = n(x, y, σ) * I(x, y)
```

(Reference: Tony Lindeberg, Feature Detection with Automatic Scale Selection, IJCV, 1998.)

As σ increases, the image becomes progressively blurrier (higher scale, lower resolution appearance). The NLoG response is computed across this whole stack.

**Illustration:** Applying NLoG to an image of a person at scales σ₀ through σ₃, the response |σ²∇²S| at a specific point (e.g. the nose) peaks at σ₁ — this is the characteristic scale for that blob. At a nearby non-blob point, no clear peak exists across scales ("no significant maximum => no blob").
...
**Level 1 — Gaussian smoothing scale (σ):**
- At a fixed image resolution, apply Gaussian blurs with increasing σ values.
...
- After covering a range of σ values within an octave, **downsample the image by factor of 2** (width and height halved).
- This starts a **new octave** — a coarser version of the image.
- In each new octave, σ starts again from a base level but represents a **higher absolute scale** because the pixel spacing is coarser.

**Structure within one octave (fixed spatial resolution):**

```
Gaussian images at scales:  σ,  sσ,  s²σ,  ...,  s^k σ
DoG images (differences):   DoG₁ = s·σ − σ,  DoG₂ = s²σ − sσ,  ...
```
...
Starting from the original image, blurs are applied at scales σ, sσ, s²σ, ..., s^k σ. Pairwise differences produce the DoG stack. Local extrema are found in the DoG stack (highlighted yellow region = the layers used for 3D extrema search). The final output is a set of **interest point candidates** (red dots scattered across the image at different DoG levels), which after thresholding become the SIFT interest points (green dots).

**Key formula — displayed blob radius:**

Each keypoint has a characteristic scale σ and comes from a specific octave. When visualizing the blob back in the original image, the circle radius is:

```
r_display ≈ √2 · σ · 2^octave
```
...
Once the DoG pyramid is built, **local extrema** are detected across the 3D scale-space (x, y, scale):
...
Detected extrema are at discrete pixel and scale locations. Lowe refines them using a **Taylor expansion** of the DoG function D(x, y, σ) around the candidate:

```
D(x) ≈ D + (∂D/∂x)^T · x + (1/2) · x^T · (∂²D/∂x²) · x
```

Setting the derivative to zero gives the refined offset x̂. If |x̂| > 0.5 in any dimension, the candidate is moved to the adjacent sample and the process is repeated.

**Rejection criteria after refinement:**

1. **Low contrast:** If |D(x̂)| < threshold (Lowe uses 0.03), the keypoint is discarded as an unstable low-contrast point.
...
The ratio of eigenvalues λ₁/λ₂ can be computed from the trace and determinant. The keypoint is rejected if:
...
After localizing the keypoint at (x*, y*, σ*), SIFT assigns a **principal orientation** to make descriptors rotation-invariant.

**Procedure:**

1. Take the **Gaussian-smoothed image** at the scale closest to σ*.
2. Compute the **gradient magnitude and direction** at each pixel in a neighborhood around the keypoint:

```
Magnitude:    m(x, y) = sqrt( (L(x+1,y) − L(x−1,y))² + (L(x,y+1) − L(x,y−1))² )

Direction:    θ(x, y) = tan⁻¹( (∂I/∂y) / (∂I/∂x) )
```
...
- Position (x*, y*)
- Characteristic scale σ* (from the DoG extremum)
- Principal orientation θ* (from the gradient histogram)
...
- **Scale invariance:** Two views of the same blob at different scales produce keypoints with different σ* values. The ratio σ₁/σ₂ equals the actual scale ratio between the two images. The descriptor region is normalized by σ* before computing.

- **Rotation invariance:** Two views of the same blob at different orientations produce keypoints with different θ* values. The descriptor is computed in the rotated frame, so rotation differences are factored out.

**Visualization (slides 47–48):** Two images of the same DVD cover, one upright and one rotated ~45°. The orange circle marks the same large blob in both images. In the upright image the orientation arrow points upward; in the rotated image the arrow has rotated accordingly. After "undoing" the rotation (normalizing by θ*), both blobs produce the same canonical patch.

**Scale ratio between images (slides 49–50):** The same flame/person blob appears at scale σ₁ in the cover photo and σ₂ in the full scene photo. The ratio of blob sizes equals σ₁/σ₂, giving a direct estimate of the scale change between the two images.
...
| **Scale space** | S(x,y,σ) = n(x,y,σ) * I(x,y); the family of images obtained by convolving I with Gaussians of increasing σ |
| **σ (sigma)** | Standard deviation of the Gaussian kernel; controls the smoothing scale |
...
| **DoG (Difference of Gaussians)** | D = G(·,sσ) − G(·,σ); efficient approximation of the normalized Laplacian of Gaussian |
| **NLoG (Normalized Laplacian of Gaussian)** | σ²∇²n_σ; the operator used for scale-normalized blob detection |
| **Characteristic scale** | σ* at which the NLoG (or DoG) response is extremized; proportional to blob size |
| **Scale-space extremum** | A DoG sample that is a local min or max among all 26 neighbors in the 3D (x,y,σ) space |
| **26-neighbor check** | The 3×3×3 neighborhood comparison: 8 in-plane + 9 above + 9 below |
| **Hessian edge test** | Uses Tr(H)²/Det(H) to reject keypoints on edges (unstable elongated structures) |
...
| **Principal orientation θ*** | The dominant gradient direction assigned to a keypoint; enables rotation invariance in descriptor computation |
| **r_display** | Visual radius of a detected blob: ≈ √2 · σ · 2^octave |
| **Ratio of blob sizes** | σ₁/σ₂; gives the relative scale factor between the same blob in two images |
...
4. **Derive the 1D blob detection principle.** Explain the σ-normalized 2nd derivative, what "characteristic scale" means, and why σ² normalization is necessary. Write the algorithm formally: given f(x), compute at k scales, find (x*, σ*) = argmax |σ² (∂²n_σ/∂x²) * f(x)|.

5. **Write the NLoG formula in 2D** and the 2D blob detection algorithm. Write the scale-space equation S(x,y,σ) = n(x,y,σ) * I(x,y).

6. **Derive why DoG ≈ (s−1)NLoG.** Write the DoG equation. Explain why DoG is used in practice instead of NLoG directly.

7. **Describe the full Gaussian/DoG pyramid construction** — explain the two types of scale (Gaussian σ and octave), how many blur levels per octave, and how DoG images are generated.
...
10. **Explain orientation assignment step-by-step:** gradient computation, 36-bin histogram with Gaussian weighting, peak selection, and the 80% rule for secondary orientations. Write the gradient direction formula θ = tan⁻¹(∂I/∂y / ∂I/∂x).
...
12. **Explain what scale and orientation give us:** why knowing both allows rotation and scale variations to be "undone," and how the ratio σ₁/σ₂ gives the scale ratio between views.
...
- **σ is not the same as octave.** σ controls Gaussian smoothing within a fixed resolution image. Octave controls image resolution (downsampling by 2). SIFT uses both simultaneously. Mixing them up is a very common error.

- **The 26-neighbor check is 3×3×3 minus 1, not 3×3×3.** Count: 9+8+9 = 26. The center point is the candidate itself.

- **Characteristic scale is proportional to blob SIZE, not to position.** σ* tells you how large the blob is, not where it is. Position comes from (x*, y*).
...
- **Blob radius in visualization includes the octave factor.** The formula r_display ≈ √2 · σ · 2^octave means blobs from coarser octaves appear larger in the original image even if their within-octave σ is the same.
...
- **Low contrast points are rejected before the edge test**, not after. Lowe's threshold is |D(x̂)| < 0.03 (using normalized 0–1 pixel values).
