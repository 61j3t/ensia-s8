...
- An image is a **2-D function** $I(x, y) : \Omega \subset \mathbb{R}^2 \to \mathbb{R}$, where the value at each point is proportional to the light energy (intensity) collected at that location.
...
- A continuous image is digitised by two orthogonal operations: **sampling** (domain $\mathbb{R}^2 \to M \times N$ grid of pixels) and **quantization** (codomain $\mathbb{R} \to \{0, 1, \ldots, 2^b - 1\}$ discrete levels).
...
- A photon is an elementary particle with no mass, travelling at the speed of light in vacuum ($c \approx 3 \times 10^8$ m/s).
...
- **Diffraction**: light bends around obstacles or spreads out after passing through a narrow slit. Diffraction through a grating produces a pattern of orders ($n = 0, \pm 1, \pm 2, \ldots$) on a screen at distance $D$, with angular separation $\theta$.
...
The full spectrum runs from gamma rays (highest frequency, shortest $\lambda \approx 10^{-16}$ m) through X-rays, UV, visible, IR, microwave, FM/AM radio, to long radio waves (lowest frequency, longest $\lambda \approx 10^8$ m). The spectrum can be described by either wavelength $\lambda$ or frequency $\nu$ (related by $c = \lambda \times \nu$).
...
The BRDF describes how much light arriving from direction $(\theta_i, \phi_i)$ is reflected toward direction $(\theta_r, \phi_r)$. Surfaces range from perfectly diffuse (Lambertian — scatters equally in all directions) to perfectly specular (mirror — single reflected direction).
...
3. **Charge accumulation** (integration over exposure time): each pixel integrates charge $Q \propto (\text{number of photons}) \times (\text{exposure time})$. Longer exposure → brighter image.
...
5. **Sampling**: the continuous leaf image is sampled on a regular $M \times N$ grid; each cell's accumulated charge gives one pixel value.
...
- **Photon**: elementary particle of light; no mass; carries energy proportional to frequency ($E = h\nu$).
...
- **BRDF**: Bidirectional Reflectance Distribution Function — $f_r(\theta_i, \phi_i, \theta_r, \phi_r, \lambda)$; describes how a surface reflects light.
...
- **Sampling**: discretisation of the image domain from $\mathbb{R}^2$ to an $M \times N$ grid.
- **Quantization**: discretisation of the image codomain from $\mathbb{R}$ to $\{0, \ldots, 2^b - 1\}$.
...
1. **Define the image as a 2-D function**: write $I : \Omega \subset \mathbb{R}^2 \to \mathbb{R}$ and explain every symbol. State what $I(x, y)$ represents physically (intensity ∝ light energy collected at $(x, y)$).
...
3. **Explain the BRDF**: write the 5 arguments $f_r(\theta_i, \phi_i, \theta_r, \phi_r, \lambda)$ and state what the function expresses. Know why it is 5-D.
...
6. **Describe the 6-step digital sensing pipeline**: Photon arrival → Photoelectric effect → Charge accumulation ($Q \propto \text{photons} \times \text{time}$) → Charge-to-voltage ($V = Q/C$) → Readout/amplification → ADC (voltage → DN). Know the formula $V = Q/C$ and the charge relation $Q \propto \text{photons} \times \text{exposure time}$.
...
8. **Define sampling and quantization with equations**:
   - Sampling: $\mathbb{R}^2 \to M \times N$ (spatial grid)
   - Quantization: $\mathbb{R} \to \{0, 1, \ldots, 2^b - 1\}$
...
10. **Compute storage size**: $M \times N \times b$ bits for grayscale; $\times 3$ for colour.
