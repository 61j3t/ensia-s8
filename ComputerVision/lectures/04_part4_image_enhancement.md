...
- c: scaling constant, chosen so that the maximum output equals the maximum allowed value (S_max):
...
*Problem:* Displays (historically CRT, but also modern monitors and projectors) are **not linear**. Their intensity-to-voltage response is a power law:
    ...
    L ∝ v^(gamma_display)     with gamma_display ≈ 2.2
...
*Consequence:* If you send a linear image to such a display, it appears **darker than intended**. The display applies a power curve with gamma ≈ 2.2, compressing dark tones.

*Solution — gamma correction:*
- Apply a gamma correction (pre-distortion) to the input signal before sending it to the display.
- The correction gamma = 1 / gamma_display. For gamma_display = 2.5, correction gamma = 1/2.5 = 0.4.
...
- The gamma-corrected image combined with the display's gamma produces an overall linear response:
      ...
      gamma_correction (0.4) + display_gamma (2.2) → approximately linear final output (≈ 1.0)

*Practical example:* A gradient from black to white:
- Original linear image fed to a gamma_display = 2.5 monitor → darker on screen.
...
A family of transformations built from **piecewise-linear functions** controlled by two points:
    ...
    Control points: (r1, s1) and (r2, s2)

The transform T(r) is a piecewise-linear function connecting:
- (0, 0) → (r1, s1): low-input slope
- (r1, s1) → (r2, s2): mid-range slope (this segment is steep to stretch the middle range)
- (r2, s2) → (L-1, L-1): high-input slope

The shape resembles an S-curve (sigmoid). By setting r1 < r2 and s1 < s2 with the middle slope being steeper than the end slopes, the midtone range is expanded (stretched) and the shadow/highlight extremes are compressed.
...
**Key insight:** The exact shape of the curve depends on the choice of (r1, s1) and (r2, s2). This is the main controllable parameter set.
...
where:
- r_k = the k-th intensity level, k = 0, 1, ..., L-1
...
M*N = total number of pixels. p_r(r_k) is the probability that a randomly chosen pixel has intensity r_k.

**Step 3:** Compute the Cumulative Distribution Function (CDF).

    T(r_k) = sum_{j=0}^{k} p_r(r_j)     [fraction of pixels with intensity <= r_k]
...
Which expands to:
    ...
    s_k = (L - 1) * sum_{j=0}^{k} (n_j / MN)
        = ((L-1) / MN) * sum_{j=0}^{k} h(r_j)
...
We want **p_s(s) = constant** (uniform output). Setting p_s(s) = 1 (normalized, s in [0,1]):

    ds/dr = p_r(r)
    s = integral from 0 to r of p_r(w) dw

This is precisely the **CDF of r**. Therefore s = CDF(r) is the unique transformation that maps any input distribution to a uniform output distribution.

In the continuous case: T(r) = (L-1) * integral from 0 to r of p_r(w) dw

This yields p_s(s) = 1/(L-1), a uniform distribution over [0, L-1].
...
**Goal:** find the threshold T that **minimizes the weighted sum of within-class variances** of the two classes:
    ...
    argmin over T of  (P1 * sigma_C1^2 + P2 * sigma_C2^2)

Definitions:
- C1 = pixels with intensity <= T (dark class)
- C2 = pixels with intensity > T (bright class)
- P1 = fraction of pixels in class C1
- P2 = fraction of pixels in class C2
- sigma_C1^2 = variance of intensities in class C1
- sigma_C2^2 = variance of intensities in class C2
...
**Algorithm:** Evaluate the criterion for every possible threshold value T = 0, 1, ..., L-2 and pick the one that minimizes P1*sigma_C1^2 + P2*sigma_C2^2.
...
- **Log transform** — s = c * log(1+r); compresses bright values, stretches dark values; reveals low-intensity details.
- **Dynamic range compression** — reducing the ratio between the largest and smallest pixel values; achieved by log transform.
- **Power-law (gamma) transform** — s = c * r^gamma; gamma < 1 brightens, gamma > 1 darkens.
- **Gamma correction** — pre-distorting pixel values with gamma = 1/gamma_display so that a non-linear display produces a perceived-linear output.
- **Display gamma** — the exponent of the power-law response of a display device (typically ≈ 2.2).
- **Image gamma** — the pre-correction gamma applied to the image before display (complement of display gamma).
- **Contrast stretching** — piecewise-linear transform controlled by two points (r1,s1) and (r2,s2) that expands the dynamic range of the midtone region.
- **Thresholding** — extreme contrast stretching; binary output: s = 0 if r <= t, s = 1 if r > t.
...
- **Histogram equalization** — transform s = T(r) = (L-1) * CDF(r) that produces a quasi-uniform output histogram.
- **CDF (Cumulative Distribution Function)** — T(r_k) = sum_{j=0}^{k} p_r(r_j); the fraction of pixels with intensity <= r_k.
...
- **Otsu thresholding** — automatic threshold selection by minimizing weighted within-class variance P1*sigma_C1^2 + P2*sigma_C2^2.
- **Within-class variance** — variance of pixel intensities within a single class (C1 or C2).
...
2. **Give the formula for each basic transform and state its effect:**
   ...
   - Log: s = c * log(1+r) where c = S_max / log(1+R_max); why +1; why it stretches dark and compresses bright
   - Gamma: s = c * r^gamma; effect of gamma < 1 vs gamma > 1 vs gamma = 1
   - Contrast stretching: piecewise-linear with control points (r1,s1), (r2,s2)
   - Thresholding: binary step at t; global vs adaptive

3. **Explain gamma correction end-to-end:** why displays are non-linear (L proportional to v^2.2); what happens without correction (image appears darker); how applying correction gamma = 1/gamma_display pre-distorts the signal so the combined pipeline is linear.
...
5. **Derive or describe histogram equalization:**
   ...
   - The final formula: s_k = (L-1) * sum_{j=0}^{k} n_j / MN
   - Why CDF produces a uniform output (density conservation law argument: p_r(r) dr = p_s(s) ds, set p_s = 1/L, integrate to get s = CDF(r)).
   - Why the discrete result is quasi-flat (not perfectly flat).

6. **Explain Otsu thresholding:** criterion argmin_T (P1*sigma_C1^2 + P2*sigma_C2^2); what P1, P2, sigma_C1, sigma_C2 mean; why minimizing within-class variance gives the best separation.
...
- **Gamma correction gamma is 1 / display_gamma, not the display gamma itself.** If the display has gamma 2.5, the correction applied to the image is gamma = 0.4 (not 2.5).
...
- **Contrast stretching control points must satisfy r1 < r2 and s1 < s2** for the transform to be monotonically increasing. Non-monotonic transforms are not valid intensity transforms (they would map different inputs to the same output, losing information).
